...
- Deploy workflow now also fires automatically on every PR open/push targeting `main`, deploying that PR's head commit so it's live for on-device testing without a manual run. A newer push cancels an in-flight deploy of a now-stale commit rather than queuing behind it, and forked PRs are blocked from ever triggering a deploy with this repo's secrets.
