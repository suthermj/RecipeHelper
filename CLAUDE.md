...
- **CI deploy:** `.github/workflows/deploy.yml` does the same build and pushes it to prod over a restricted, non-root `deploy` SSH user — see `deploy/remote/README.md` for the one-time VM setup and required `DEPLOY_SSH_KEY` secret. **Fires automatically** on every PR open/push targeting `main` that touches source code (deploys that PR's head commit — every PR branch is live on production the moment you push a code change to it, no manual step). **PR-triggered runs pause for manual approval first** — the `require-approval` job targets the `production-pr-approval` GitHub Environment (required reviewers, no secrets attached), and the `deploy` job doesn't proceed until that's approved from the Actions tab. Non-source changes (`**.md`, `.gitattributes`/`.gitignore`, `.github/**`, `deploy/**`, `RecipeHelper.Tests/**`, `RecipeHelper/tests/**` — none of these are part of what `dotnet publish` ships) are skipped via `paths-ignore` and don't trigger a redeploy; a push mixing source with any of these still deploys. Also runnable manually anytime from the Actions tab (works fine from the GitHub mobile app) — no inputs, deploys whichever branch is picked in the "Use workflow from" selector, e.g. to redeploy `main` itself or a branch with no open PR; **these `workflow_dispatch` runs skip the approval gate entirely** (so redeploying `main` after a merge never waits on a reviewer). A newer push cancels an in-flight deploy of a now-stale commit (`cancel-in-progress: true`) rather than queuing behind it. Guarded against forked PRs and Dependabot PRs ever running with this repo's deploy secrets — Dependabot branches are same-repo, so they don't need review to reach a build/CodeQL check, but they're excluded from auto-deploy so an unreviewed dependency bump can't go straight to prod.
