...
- **Auth: intentionally none, for now** — left open at the user's request rather than gated behind the shared-secret token it originally shipped with. Not linked from anywhere outside the app itself. If this needs to be locked back down later, the token + long-lived-cookie approach (`AdminSettings:LogsToken`, `404` instead of `401` so the route doesn't advertise itself) is preserved in git history on `AdminController.cs` / `Views/Admin/Logs.cshtml` prior to that change.
- **One-time VM setup required** (not yet done as of writing this section — do this manually, this app's session tooling can't reach the VM): the service runs as `www-data`, which by default can't read the systemd journal — grant it: `sudo usermod -aG systemd-journal www-data && sudo systemctl restart recipehelper` (group membership only takes effect for processes started after the change). Without this, the page shows journalctl's permission-denied stderr with this same hint inline.
...
- **Settings entry point:** `Settings/Index` shows a "System Logs" row (under a "System" section) linking to `/Admin/Logs`.
