...
| `Controllers/AdminController.cs` | Mobile log viewer (`/Admin/Logs`) — tails `journalctl -u recipehelper` for on-phone debugging; see Observability section |
...
- **CI deploy:** `.github/workflows/deploy.yml` does the same build and pushes it to prod over a restricted, non-root `deploy` SSH user — see `deploy/remote/README.md` for the one-time VM setup and required `DEPLOY_SSH_KEY` secret. **Fires automatically** on every PR open/push targeting `main` that touches source code (deploys that PR's head commit — every PR branch is live on production the moment you push a code change to it, no manual step). Non-source changes (`**.md`, `.gitattributes`/`.gitignore`, `.github/**`, `deploy/**`, `RecipeHelper.Tests/**`, `RecipeHelper/tests/**` — none of these are part of what `dotnet publish` ships) are skipped via `paths-ignore` and don't trigger a redeploy; a push mixing source with any of these still deploys. Also runnable manually anytime from the Actions tab (works fine from the GitHub mobile app) — no inputs, deploys whichever branch is picked in the "Use workflow from" selector, e.g. to redeploy `main` itself or a branch with no open PR. A newer push cancels an in-flight deploy of a now-stale commit (`cancel-in-progress: true`) rather than queuing behind it. Guarded against forked PRs and Dependabot PRs ever running with this repo's deploy secrets — Dependabot branches are same-repo, so they don't need review to reach a build/CodeQL check, but they're excluded from auto-deploy so an unreviewed dependency bump can't go straight to prod.
...
- **Data protection keys persist across deploys.** `Program.cs` calls `AddDataProtection().PersistKeysToFileSystem("/var/lib/recipehelper/keys")`, and `deploy-recipehelper.sh` creates/chowns that directory on every deploy — antiforgery tokens (and any other cookie signed via Data Protection) survive a restart.
...
### Mobile log viewer (`/Admin/Logs`)

Quick on-phone way to tail production logs without SSH or opening Grafana. `AdminController` shells out to `journalctl -u recipehelper` and serves the output as plain text; `Views/Admin/Logs.cshtml` is a standalone (`Layout = null`) dark, mobile-first page that polls it every 5s.

- **Auth:** no user/identity system exists anywhere else in the app, so this route is gated by a single shared-secret token instead — `AdminSettings:LogsToken` in `appsettings.Production.json` on the VM (`/etc/recipehelper/appsettings.Production.json`, survives deploys per `deploy-recipehelper.sh`). Visit `/Admin/Logs?token=<the-token>` once; a long-lived (`1y`), `HttpOnly`/`Secure` cookie carries it after that, so the bare URL can be added to the iOS home screen. An unset or wrong token gets a `404` (not `401`), so the route doesn't advertise its own existence to a scan.
- **One-time VM setup required** (not yet done as of writing this section — do this manually, this app's session tooling can't reach the VM):
  1. Add a `LogsToken` value to `/etc/recipehelper/appsettings.Production.json`'s `AdminSettings` section (create the section if absent) and restart the service so it picks it up.
  2. The service runs as `www-data`, which by default can't read the systemd journal — grant it: `sudo usermod -aG systemd-journal www-data && sudo systemctl restart recipehelper` (group membership only takes effect for processes started after the change). Without this, the page shows journalctl's permission-denied stderr with this same hint inline.
- **Endpoints:** `GET /Admin/Logs` (the page) and `GET /Admin/LogsData?lines=&grep=` (plain-text data the page polls via `fetch()` — deliberately not part of the page navigation itself, since `sw.js` stale-while-revalidates navigations but a same-origin `fetch()` always hits the network).
- **Settings entry point:** `Settings/Index` shows a "System Logs" row (under a "System" section) linking to `/Admin/Logs`, gated by `SettingsVM.LogsAvailable` (`AdminSettings:LogsToken` configured) so it's hidden until the one-time VM setup above is done. The link itself carries no token — it only works once the cookie from a prior token-authenticated visit is already set, same as the bookmark flow.
