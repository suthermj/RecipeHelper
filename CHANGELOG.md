...
- Recipe images now upload with a long-lived, immutable `Cache-Control` header (`public, max-age=31536000, immutable`, same value already used for fingerprinted static assets), so repeat page loads serve them from the browser's HTTP cache instead of re-fetching from Blob Storage every time. `StorageService.BackfillImageCacheHeadersAsync`, runnable via `dotnet run -- --backfill-image-cache-headers`, one-time backfills this header onto images uploaded before this change went in. (#54)
- Recipe images are also resized (max 2000px) and recompressed (JPEG quality 88) server-side on upload, mirroring the approach already used for photo-import — reduces per-image transfer size over the transatlantic Hetzner (Germany) ↔ user (US) path on first load. (#54)
- `StorageService.BackfillCompressExistingImagesAsync`, runnable via `dotnet run -- --backfill-compress-images`, one-time recompresses existing recipe images uploaded before the resize/recompress step above went in. Since blobs are served with an `immutable` Cache-Control, each shrunk image is uploaded under a *new* blob URL rather than overwritten in place (an immutable response promises its URL's content never changes) — the backfill repoints every `Recipe.ImageUri` that referenced the old URL, and deliberately leaves old blobs in place rather than deleting them. (#54)
- Imported recipes now re-host their image onto our own Blob Storage at import time (`ImportService.RehostExternalImageAsync`) instead of linking the recipe source site's URL directly — gets the same resize/recompress + immutable Cache-Control as a manual upload, and stops page loads depending on some random blog's or Spoonacular's own server speed. Falls back to the source URL if the download fails (e.g. a site blocking non-browser requests) rather than leaving the recipe with no image. `ImportService.BackfillRehostExternalImagesAsync`, runnable via `dotnet run -- --backfill-rehost-external-images`, one-time re-hosts existing recipes' external images the same way. (#54)
